...
function MyFavorites(props: propType) {
  const { prodData, isFav } = props;
  const { data: session, status } = useSession();
  const dispatch = useAppDispatch();

  const favClickAction = () => {
    if (!prodData) return;
    ...
    if (isFav) {
      dispatch(removeFavorite({ vendor: prodData, token: session.backendToken }));
    } else {
      dispatch(addFavorite({ vendor: prodData, token: session.backendToken }));
    }
  };
    ...
    <div className="fav-container">
      ...
      >
        <svg
          ...
          height={14}
          width={14}
          className={`fav-icon ${isFav ? "fav" : ""}`}
          fill={isFav ? "red" : "black"}
        >
