...
import { getEssentialOrders } from "@/app/store/features/essentialOrderSlice";
...
import { EssentialOrderSummary } from "@/app/types/essentialOrder";
...
const OrdersListPage: React.FC = () => {
  ...
  const {
    ...
    isLoading: isLoadingEssentialOrders,
    error: essentialOrdersError,
  } = useAppSelector((state: RootState) => state.essentialOrderState);

  useEffect(() => {
    if (session?.backendToken) {
      dispatch(getOrders({ token: session.backendToken }));
      dispatch(getEssentialOrders({ token: session.backendToken }));
    }
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.section}>
        <h2 className={styles.sectionHeading}>Renovation Orders</h2>
                  ...
                  <Link href={`/orders/${order.id}`} className={styles.card}>
                    ...
                    <div className={styles.cardFooter}>
                      <span className={styles.meta}>
                        ...
                        {order.itemCount !== 1 ? "s" : ""} · {formattedDate}
                      </span>
      ...
      <section className={styles.section}>
        <h2 className={styles.sectionHeading}>Room Orders</h2>

        {isLoadingEssentialOrders && <OrdersListSkeleton />}

        {!isLoadingEssentialOrders && essentialOrdersError && (
          <ErrorState
            variant="inline"
            title="Couldn't load your room orders"
            message={essentialOrdersError}
          />
        )}

        {!isLoadingEssentialOrders &&
          !essentialOrdersError &&
          essentialOrders.length === 0 && (
            <div className={styles.empty}>
              <span className={styles.emptyIcon} aria-hidden="true">
                🛋️
              </span>
              <p className={styles.emptyText}>No room orders yet</p>
              <p className={styles.emptySubtext}>
                Shop a curated room and check out to see it here.
              </p>
              <Link href="/roomCategory" className={styles.emptyAction}>
                Shop by room
              </Link>
            </div>
          )}
        ...
        {!isLoadingEssentialOrders &&
          ...
          essentialOrders.length > 0 && (
            <ul className={styles.list} role="list">
              {essentialOrders.map((order: EssentialOrderSummary) => {
                const formattedTotal = new Intl.NumberFormat("en-IN", {
                  style: "currency",
                  currency: "INR",
                  maximumFractionDigits: 0,
                  ...
                  <li key={order.id}>
                    ...
                    >
                      ...
                      <p className={styles.vendorName}>
                        {order.vendorCount} vendor
                        {order.vendorCount !== 1 ? "s" : ""}
                      </p>
                      <div className={styles.cardFooter}>
                        <span className={styles.meta}>
                          {order.itemCount} item
