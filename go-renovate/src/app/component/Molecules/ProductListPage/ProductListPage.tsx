...
function ProductListPage(props: {
  ...
  const searchQuery = searchParams.get(SEARCH_PARAM)?.trim() || "";
  const rawFilterQuery = buildVendorsQueryString(searchParams);
    ...
    <div className="product-page-container">
      ...
      <div className="product-page-list">
        {searchQuery ? (
          <div className="search-results-header">
            ...
            <h1 className="search-results-title">
              Results for <span className="search-results-query">&ldquo;{searchQuery}&rdquo;</span>
            </h1>
            <Link href="/vendors" className="search-results-clear">
              Clear search
            </Link>
          </div>
        ) : (
        ...
        {productLists?.isloading && !hasCatalog ? (
          ...
          <ErrorState
            title={searchQuery ? "No vendors match your search" : "No vendors match your filters"}
            message={
              searchQuery
                ? "Try a different term, or clear a filter to broaden the results."
                : "Try clearing a filter or choosing a different combination."
            }
          />
        ) : (
          <ProductList
